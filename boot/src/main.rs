...
use std::{path::Path, process::Command}; // new
...
pub fn main() {
    ...
    let kernel_binary =
        Path::new("target/x86_64-blog_os/debug/blog_os").canonicalize()
                                                        .unwrap();
    ...
    let out_dir = kernel_binary.parent().unwrap(); // create a new build command; use the `CARGO` environment variable to
